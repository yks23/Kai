"""
持久化用户配置 — 按平台存储:
  Windows: %APPDATA%\\machine\\settings.json
  macOS/Linux: ~/.config/machine/settings.json (XDG 约定)

支持的配置项:
  base_dir  — 工作区目录 (machine base <path>)
  cli_name  — CLI 命令名 (machine name <name>)
"""
import json
import os
import shutil
...
def _config_dir() -> Path:
    """按平台返回配置目录。Windows: APPDATA/machine；macOS/Linux: ~/.config/machine"""
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        if appdata:
            return Path(appdata) / "machine"
        # 回退到用户目录下 .config/machine
        return Path.home() / ".config" / "machine"
    return Path.home() / ".config" / "machine"
...
_LEGACY_SETTINGS_FILE = _legacy_config_dir() / "settings.json"

# 默认值
_DEFAULTS = {
    "base_dir": "",       # 空 = 使用 CWD
    "cli_name": "machine",    # 默认命令名
    "model": "Auto",      # 默认模型
...
def get_cli_name() -> str:
    """获取当前 CLI 命令名"""
    return load_settings().get("cli_name", "machine")
...
def set_cli_name(new_name: str):
    ...
    if not bin_dir:
        ...
        print(f"      alias {new_name}='machine'")
        s = load_settings()
        s["cli_name"] = new_name
        save_settings(s)
        return False
    ...
    src = _resolve_exe(bin_dir, "machine") or _resolve_exe(bin_dir, "kai") or _resolve_exe(bin_dir, "secretary") or _resolve_exe(bin_dir, old_name)
    if src is None:
        src = bin_dir / "machine"  # 占位，下面 src.exists() 为 False 会走 _create_wrapper_script
    ...
    try:
        ...
        print(f"   ⚠️ 创建链接失败: {e}")
        print(f"   💡 请手动执行: alias {new_name}='machine'")
...
def _has_kai_or_secretary(directory: Path) -> bool:
    """检查目录下是否存在 machine / kai / secretary 可执行文件（含 .exe）"""
    for base in ("machine", "kai", "secretary"):
...
def _find_bin_dir() -> Path | None:
    ...
    # 方法4: 从 PATH 搜索
    for name in ["machine", "kai", "secretary"]:
...
# Auto-generated by machine name command
