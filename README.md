...
- 使用方式二选一：
  - **将 kai 加入 PATH**：通过 `pip install -e .` 或 `pip install kai` 安装后，在对应环境的 `Scripts`（Windows）或 `bin`（Unix）目录在 PATH 中即可直接运行 `kai`
...
适用于仅使用、不改源码。发布后包名为 **kai**，安装后可在任意目录使用 `kai`（需保证安装目录在 PATH 中）。

```bash
# 用户级安装（推荐，无需 root）
pip install --user kai
...
- 建议在项目目录或工作区使用 **venv**：`python -m venv .venv`，再 `pip install -e .` 或 `pip install kai`，这样 `kai` 仅在该环境中存在，避免与其它项目冲突。
...
| 输入流   | `machine learn-stream-auth --open-login` | 从本机浏览器自动导入网络学堂 Cookie/CSRF 到配置，降低手工复制门槛。 |
...
- **前台执行**（阻塞直到结束或用户退出）：`task`、`hire`、`fire`、`workers`、**`monitor`**（TUI 或 `--text`/`--once`）、`report`、`base`、`name`、`model`、`target`、`help`、`skills`、`learn`、`forget`、`use`、`stop`、`clean-*`。`check` 为前台 tail 日志（按 q 或 Ctrl+C 退出）。
- **后台执行**（输出写入日志，不阻塞）：`kai start <worker|kai>`、`kai keep "目标"`、`kai recycle`。
- **日志位置**：
  - **Kai 相关**（秘书/扫描器/keep）→ `agents/kai/logs/`：`scanner.log`（kai 扫描器）、`secretary.log`（经秘书提交的子进程）、`keep.log`（keep 模式）。
  - **Worker 相关** → `agents/<worker名>/logs/scanner.log`。
- 查看日志：`kai check kai`（scanner）、`kai check keep`（keep）、`kai check <worker名>`（该 worker 的 scanner）。
...
- 工作区下的 `agents/`（含 `agents/kai/` 的 report、stats、logs、solved-report、unsolved-report）、`skills/`；
- 秘书记忆与目标：`agents/kai/goals.md`、`agents/kai/memory.md`（可由 `kai target` 等写入）。
...
若你通过 **PyPI** 安装（`pip install kai`），与「从源码开发安装」的差异在于：

- **无需克隆仓库**：在任意目录执行 `pip install kai`（或 `pip install --user kai`）即可，安装后 `kai` 在对应环境的 Scripts/bin 中。
- **不涉及源码**：无法直接改 kai 的代码；升级依赖 `pip install -U kai`（见下方「如何升级 kai」）。
...
安装 **kai** 后（或开发安装并激活对应环境后），只要 `kai` 在 PATH 中，即可在任意目录使用：
...
- **工作区**：首次使用建议在项目根目录执行 `kai base .`，之后无论在哪执行 `kai`，都会操作该目录下的 `agents/`（含 kai 的 report、stats、logs 等）、`skills/` 等。
...
- **虚拟环境**：在对应 venv/conda 中执行 `pip install -U kai` 即可，该环境内的 `kai` 会更新。
- **从源码开发安装**：在仓库根目录执行 `git pull` 后无需重装（`pip install -e .` 已指向当前源码），拉取即是最新代码。

### 获知新版本与发布说明

- **PyPI**：[https://pypi.org/project/kai/](https://pypi.org/project/kai/) 可查看最新版本号与简要说明。
...
- **用户级安装**：在当初执行 `pip install --user kai` 的同一用户、同一 Python 环境下执行上述命令即可。
- **虚拟环境**：先激活对应 venv/conda，再执行 `pip uninstall kai`，仅会从该环境中移除，不影响系统或其他环境。
...
| 路径 | 说明 |
|------|------|
| `agents/` | 所有 Agent（kai + 各 worker）。 |
| `agents/kai/tasks/`、`assigned/` | Kai 待处理/已分配任务。 |
| `agents/kai/report/` | Worker 完成的报告（待回收者审查）。 |
| `agents/kai/stats/` | 调用统计与对话日志。 |
| `agents/kai/logs/` | Kai 相关日志（scanner、secretary、keep）。 |
| `agents/kai/solved-report/`、`unsolved-report/` | 回收者判定为已解决/未解决的报告。 |
| `agents/<name>/tasks/`、`ongoing/`、`logs/` | 各 worker 的待处理、执行中、日志。 |
| `skills/` | 学会的技能（可复用任务模板）。 |
| `agents/kai/goals.md`、`memory.md` | 秘书记忆与全局目标（kai target 等）。 |

---

## 工作流程简述

1. **提交**：`kai task "..."` → 秘书 Agent 读入请求，决定新建或追加 → 写入某工人的 `agents/<name>/tasks/`。
2. **扫描**：`kai start <name>` 启动的扫描器轮询该工人的 `tasks/`，将 .md 移到 `ongoing/`。
3. **执行**：对每个 `ongoing/*.md` 反复调用 Worker Agent；Worker 可更新同一文件记录进展。
4. **完成**：Worker 认为完成后删除该 `ongoing/*.md`，在 `agents/kai/report/` 写入 `*-report.md`；回收者（`kai recycle`）可审查并移至 `agents/kai/solved-report/` 或 `unsolved-report/`。
...
- `.cursor/rules/kai.md` 等规则文件可定义秘书/Worker 的全局规则，供 Cursor 使用。
...
Worker 未完成时会在文件末尾追加 `## 进展记录`；完成后会删除该文件并在 `agents/kai/report/` 写入报告。
...
3. **上传到 PyPI**：
   ...
   - **Test PyPI（可选）**：先验证再发正式版时，可使用 `twine upload --repository testpypi dist/*`，然后用户可通过 `pip install -i https://test.pypi.org/simple/ kai` 安装测试版。

4. **发布后**：在 GitHub 可创建对应 Release、附上 tag 与发布说明；用户即可通过 `pip install -U kai` 获取新版本。
